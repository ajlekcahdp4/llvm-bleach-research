...
+ Соглашением о вызовах (ABI -- Application Binary Interface) называется документ,
  ...
  Описание ABI стандартизирует как функция ожидает получить свои аргументы и как
  она возвращает свой результат (через выделенный регистр или програмный стек).
  Соглашение о вызовах также значительно отличаются даже внутри одной архитектуры.
  Например программа, написанная под RISC-V процессор с LP64D не будет работать
  для RISC-V с LP64F.
...
Являясь развитой компиляторной инфраструктурой LLVM предосталяет большое число
...
для всех реализованных оптимизаций. Всё это позволяет переиспользовать
внутренние оптимизации кода (как LLVM IR, так и MIR) из внешних проектов.
