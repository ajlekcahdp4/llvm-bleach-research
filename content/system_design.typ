...
В данной главе мы обсудим возможные решения проблем существующих лифтеров в
LLVMsys IR. Отличительной чертой предложенного подхода будет являться
минимализация ручного описания архитектуры.
...
инструменты поддерживают собственное описание инструкций для работы с машинным
...
SPIRV). LLVM обладает информацией о кодировках, операндах, семантике ассемблера
...
снижение вероятности ошибки. Все поддержанные в LLVM расширения косвенно
тестируются компилятором `clang`. Это означает, что если в описании некого
расширения допущена ошибка, то во время компиляции под архитектуру процессора,
...
== Универсальная модель инструкции RISC-V <chapt-instr-model>
...
как описано в @mir-desc) и дальнейшего описания всех инструкций в фиксированном
для каждого из типов формате. Итого предлагается заменить обработку подъёмаsys
каждой инструкции (число которых измеряется в сотнях) на обработку нескольких
классов инструкций и дальнейшей шаблонной подстановки LLVM IR кода,
соответствующего каждой инструкции из класса. Такой подход сильно упрощает
...
решения будет неизменна. Для осознания принципа разберём самый простой и
наиболее часто встречающийся класс инструкций -- арифметические инструкции или
работа с памятью. Данный класс инструкций предлагается назвать "регулярным" и
для сопоставления LLVM IR кода инструкциям этого класса предлагается
использовать функции LLVM IR. Другими словами, каждой инструкции, например `ADD`,
мы сопоставляем определение функции на LLVM IR по следующему принципу:
- Каждый входной операнд (регистр или число) становится аргументом этой функции.
- Каждый выходной операнд-регистр (все инструкции имеют не более одного)
  становится возвращаемым значением этой функции.
...
входных регистров (`X11` и `X12`) как аргументы вызова и сохраним возращаемое
...
#figure(caption: [Поднятая в LLVM IR инструкция `ADD`])[
#set text(size: 1.2em)
```LLVM
  ...
...
== Собственное соглашение о вызовах <chapt-abi-concept>
...
собой ABI исходного кода (Аналогично представленному в
@bleach-conference-paper).
...
Далее каждой функции присвоим сигнатуру, в которой она принимает указатель
(ссылку) на наш класс текущего контекста и не возвращает ничего. Иными словами,
...
соглашении о вызовах аргументы функции хранятся в регистрах `X10-X15`, а
...
регистрами являлись `X5-X10`, то в поднятии кода ничего бы не изменилось,
вызывающая функция в таком случае обновила бы значения регистров с пятого по
десятый в контексте, а вызываемая использовала бы значения этих регистров из
контекста. То же верно и для возвращаемого значения функции, регистр
возращаемого значения будет обновлён в контексте вызываемой функцией и
интерпретирован вызывающей функцией как результат.
...
Объём ручного описания архитектуры сведён к минимуму и включает только
